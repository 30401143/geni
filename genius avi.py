...
os.system('clear')
output = render('GENIUS', colors=['red', 'white'], align='center')
print(output)
import random,requests,time;from uuid import uuid4;from AegosPy import GetInfoInsta;uid= uuid4();from rich.console import Console;from rich.table import Table;import threading
import random,requests,time;from uuid import uuid4;from AegosPy import GetInfoInsta;uid= uuid4();from rich.console import Console;from rich.table import Table;import threading
import datetime,sys  #هدني المكاتب لتشغيل سورس توقيت
...
g= datetime.datetime(2024, 2, 24, 00, 00 ,0) #هنا تقدر تغير الوقت بالأداة

if (x.strftime("%x"))>(g.strftime("%x")):
 print('\n\n')
 print("     "+' انتهت الصلاحيه عليك بمراسله المطورللحصول على احدث نسخه insta : @30401143 ' )
 print('\n\n')
 print(x)
...
table.add_column("The Tool By : - 𝐆𝐞𝐧𝐢𝐮𝐬 | ",style='green')
table.add_row("My Account : @30401143",style='yellow')
console = Console()
console.print(table)
...
try:
	...
	I = input(f'{B} - Enter ID Tele - :{C} ')
	
	T = input(f'{B}- Enter Token Tele - : {F}')
	see = input(f' {B}- Enter Sessoined - :{C} ')
	os.system('clear')
except:
	print('ERROR PUT')
	exit()
